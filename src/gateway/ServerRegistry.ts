...
export class ServerRegistry {
  ...
  register(name: string, server: IMCPServer): void {
    if (this.servers.has(name)) {
      console.warn(`[ServerRegistry] 服务 '${name}' 已存在，将被覆盖`);
    }

    this.servers.set(name, server);
    this.serverStatuses.set(name, server.getStatus());
    console.log(`[ServerRegistry] 已注册服务: ${name} (版本: ${server.version})`);
  }
  ...
  unregister(name: string): void {
    if (this.servers.delete(name)) {
      ...
      console.log(`[ServerRegistry] 已注销服务: ${name}`);
    }
  }
  ...
  private startHealthCheck(): void {
    ...
    console.log('[ServerRegistry] 已启动健康检查 (间隔: 30秒)');
  }
  ...
  private async performHealthCheck(): Promise<void> {
    const checkPromises = Array.from(this.servers.entries()).map(async ([name, server]) => {
      try {
        ...
        console.error(`[ServerRegistry] 服务 '${name}' 健康检查失败:`, error);
  ...
  stopHealthCheck(): void {
    if (this.healthCheckInterval) {
      ...
      console.log('[ServerRegistry] 已停止健康检查');
    }
  }
