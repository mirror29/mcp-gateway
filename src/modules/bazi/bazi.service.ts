...
/**
 * 八字算命服务
 * 实现IMCPServer接口，提供专业的八字算命计算服务
 */
@Injectable()
export class BaziService implements IMCPServer {
  readonly name = 'bazi';
  readonly version = '1.0.0';
  readonly description = '专业八字算命计算服务，基于传统中华命理学';
  readonly tools = ['getBaziDetail', 'getBaziFortune', 'getCompatibility', 'getLuckyInfo'];
  ...
  async executeTool(toolName: string, params: any): Promise<any> {
    ...
    try {
      this.logger.debug(`执行工具: ${toolName}`, params);
      ...
      this.logger.debug(`工具执行完成: ${toolName} (${responseTime}ms)`);
